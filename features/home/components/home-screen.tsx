...
export const HomeScreen = ({ hasActiveTrip = true }: HomeScreenProps) => {
  return (
    <MobileShell className="bg-surface-gray">
      <div className="flex flex-1 flex-col gap-4 px-5 pt-3 pb-4">
        <div className="flex items-center justify-between">
          <div className="flex flex-col gap-0.5">
            <p className="text-text-secondary-soft text-sm font-medium">{CURRENT_USER.greetingName}님, 안녕하세요</p>
            <h1 className="text-ink-900 text-[22px] font-bold tracking-[-0.5px]">이번엔 어디로 떠나볼까요?</h1>
          </div>
          <button type="button" aria-label="알림" className="flex size-11 items-center justify-center opacity-70">
        ...
        {hasActiveTrip ? (
          <Link
            href={`/trips/${ACTIVE_TRIP.id}/invite`}
          ...
          >
            <span className="border-line-hairline inline-flex h-5 w-fit items-center gap-1.5 rounded-2xl border bg-white px-2">
              ...
              <span className="text-text-secondary-soft text-xs font-bold">{ACTIVE_TRIP.dDayLabel}</span>
            </span>
            <p className="text-text-basic text-[22px] font-bold tracking-[-0.5px]">{ACTIVE_TRIP.title}</p>
            <div className="flex items-center justify-between">
              <div className="flex items-center">
                {ACTIVE_TRIP.members.map((member, index) => (
                  <Avatar
                    key={member}
                    member={member}
                    size="sm"
