...
// 미니게임은 일정이 확정된 뒤(OngoingTripHomeScreen)에만 노출한다.
const HOST_MENU = [
  { key: 'schedule', label: 'AI 일정', description: '일차별 동선 확인 · 일정 확정', path: 'schedule' },
    ...
    <MobileShell className="bg-surface-gray">
      <div className="flex flex-1 flex-col gap-4 px-5 pt-3 pb-4">
        ...
        {!isGuest && trip.phase !== 'ongoing' && trip.phase !== 'settled' ? (
          <p className="text-text-secondary-soft text-center text-[12.5px] font-medium">
            일정을 확인하고 &apos;AI 일정&apos;에서 확정하면 여행이 시작돼요
          </p>
        ) : null}
      </div>
      <TabBar />
